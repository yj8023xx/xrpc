...
public class RpcRequestHandler implements RequestHandler<TinyRpcPacket>, ServiceProviderRegistry {
    ...
    public TinyRpcPacket handle(TinyRpcPacket packet) {
        ...
        RpcRequest rpcRequest = (RpcRequest) ProtostuffSerializerUtil.deserialize(packet.getPayload(), RpcRequest.class);
        try {
            ...
            if (serviceProvider != null) {
                ...
                // 把结果封装成响应命令并返回
                return new TinyRpcPacket(new TinyRpcResponseHeader(TinyRpcMessageType.RPC_RESPONSE.getVal(), SerializeUtil.DEFAULT_SERIALIZE_TYPE, header.getRequestId()), ProtostuffSerializerUtil.serialize(result));
            }
            // 如果没找到，返回NO_PROVIDER错误响应
            logger.warn("No service provider of {}#{}(String)!", rpcRequest.getInterfaceName(), rpcRequest.getMethodName());
            return new TinyRpcPacket(new TinyRpcResponseHeader(TinyRpcMessageType.RPC_RESPONSE.getVal(), header.getRequestId(), TinyRpcCode.NO_PROVIDER.getVal(), "No Provider!"), new byte[0]);
        } catch (Throwable t) {
            // 发生异常，返回UNKNOWN_ERROR错误响应
            logger.warn("Exception: ", t);
            return new TinyRpcPacket(new TinyRpcResponseHeader(TinyRpcMessageType.RPC_RESPONSE.getVal(), header.getRequestId(), t), new byte[0]);
        }
    }

    @Override
    public int type() {
        return TinyRpcMessageType.RPC_REQUEST.getVal();
    }
