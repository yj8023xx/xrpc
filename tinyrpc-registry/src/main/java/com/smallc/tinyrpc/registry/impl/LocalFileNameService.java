...
public class LocalFileNameService implements NameService {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "rw");
            ...
            try {
                ...
                if (fileLength > 0) {
                    ...
                    metadata = SerializeUtil.deserialize(bytes, Metadata.class, SerializeType.HESSIAN.getVal());
                } else {
                ...
                bytes = SerializeUtil.serialize(metadata, SerializeType.HESSIAN.getVal());
    ...
    @Override
    public URI getServiceAddress(String serviceName) throws IOException {
        Metadata metadata;
        try (RandomAccessFile raf = new RandomAccessFile(file, "rw");
             FileChannel fileChannel = raf.getChannel()) {
            FileLock lock = fileChannel.lock();
            try {
                ...
                metadata = bytes.length == 0 ? new Metadata() : SerializeUtil.deserialize(bytes, Metadata.class, SerializeType.HESSIAN.getVal());
                logger.info(metadata.toString());
            } finally {
                lock.release();
            }
